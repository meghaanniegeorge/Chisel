...
#endif
#include <iostream>
class PWM_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    PWM_api_t(VPWM* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_fall)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rise)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["PWM.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VPWM* top = new VPWM;
    std::string vcdfile = "test_run_dir/PWM/PWM.PWMVerilatorTester349364187/PWM.vcd";
